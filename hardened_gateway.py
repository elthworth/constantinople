...
class IntelligentRouter:
    ...
    def report_success(self, miner: MinerInfo, ttft_ms: float, tps: float):
        ...
        n = miner.requests_served
        miner.avg_ttft_ms = miner.avg_ttft_ms * (n - 1) / n + ttft_ms / n
        miner.avg_tps = miner.avg_tps * (n - 1) / n + tps / n
    ...
    async def health_check_dead_miners(self, session: aiohttp.ClientSession = None):
        ...
        that leak connectors. Falls back to creating a temporary session if none provided.
        """
        ...
        for miner in dead_miners:
            ...
            # Wait at least 30s before retrying
            ...
            try:
                if session:
                    async with session.get(
                        ...
                        if resp.status == 200:
                            miner.alive = True
                            miner.reliability_score = 0.3  # Start cautious
...
def create_gateway_app(validator: HardenedGatewayValidator) -> FastAPI:
    ...
    app = FastAPI(title="Inference Subnet Gateway", version="2.0.0")
    ...
    async def chat_completions(
        ...
        if request.stream:
            return StreamingResponse(
                _stream_response(validator, prompt, request.max_tokens, request.model, session_id, messages=messages, sampling_params=sampling_params),
        ...
        finish = "length" if out_tokens >= request.max_tokens else "stop"
    ...
    async def verify_monitoring_key(request: Request):
        ...
        if auth[:7].lower() == "bearer ":
            ...
            if _timing_safe_key_in(key, validator.config.MONITORING_KEYS):
                return key
            # Also accept inference API keys for monitoring
            if validator.config.API_KEYS and _timing_safe_key_in(key, validator.config.API_KEYS):
                return key
    ...
    async def health(request: Request):
        ...
        if validator.config.MONITORING_KEYS:
            ...
            if auth[:7].lower() == "bearer ":
                key = auth[7:].strip()
                all_keys = validator.config.MONITORING_KEYS | validator.config.API_KEYS
                if _timing_safe_key_in(key, all_keys):
                    is_authed = True
    ...
    async def completions(
        ...
        result = await validator.process_request(
            request.prompt, max_tokens=request.max_tokens, is_synthetic=False,
        )
        if result is None:
            raise HTTPException(status_code=503, detail="No miners available")
    ...
    async def dashboard():
        ...
        html = _DASHBOARD_HTML.replace("{{MODEL_NAME}}", validator.model.config.name)
